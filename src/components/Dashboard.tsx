...
export default function Dashboard({ plants, zones, tasks }: DashboardProps) {
  ...
  return (
    <div className="space-y-6">
            ...
            {pendingTasks.slice(0, 5).map(task => (
              <div key={task.id} className="p-3 rounded-lg bg-gray-50 border border-gray-100">
                ...
                <span className={`inline-block mt-2 text-xs px-2 py-0.5 rounded-full font-medium ${
                  task.type === 'watering' ? 'bg-blue-100 text-blue-700' :
                  task.type === 'fertilizing' ? 'bg-green-100 text-green-700' :
                  task.type === 'pruning' ? 'bg-purple-100 text-purple-700' :
                  task.type === 'transplanting' ? 'bg-amber-100 text-amber-700' :
                  'bg-gray-100 text-gray-700'
                }`}>
                  {task.type === 'watering' ? '💧 Полив' :
                   task.type === 'fertilizing' ? '🧪 Подкормка' :
                   task.type === 'pruning' ? '✂️ Обрезка' :
                   task.type === 'transplanting' ? '🔄 Пересадка' :
                   '🔍 Осмотр'}
                </span>
              </div>
            ))}
      ...
      {/* Health Overview */}
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h3 className="text-lg font-semibold text-gray-800 mb-4">Состояние здоровья</h3>
        <div className="flex gap-2 flex-wrap">
          {plants.map(plant => (
            <div
              key={plant.id}
              className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-100 hover:shadow-md transition-shadow cursor-default"
              title={`${plant.name} — ${plant.latinName}`}
            >
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: HEALTH_COLORS[plant.health] }} />
          ...
          {Object.entries(HEALTH_COLORS).map(([key, color]) => (
            <div key={key} className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-500">
                {key === 'excellent' ? 'Отличное' : key === 'good' ? 'Хорошее' : key === 'fair' ? 'Удовл.' : key === 'poor' ? 'Плохое' : 'Крит.'}
                {' '}({plants.filter(p => p.health === key).length})
              </span>
            </div>
          ))}
        </div>
